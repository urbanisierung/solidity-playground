  ...
  public compile() {
    ...
    if (!compiledSources) {
      ...
      if (compiledSources.errors) {
        ...
        // console.log(compiledSources.contracts)

        for (let contractName in compiledSources.contracts[this.contract]) {
          // const contractName = contractFileName.replace('.sol', '')
          console.log('Writing: ', `${contractName}.json`)
          fs.outputJsonSync(
            path.resolve(buildPath, `${contractName}.json`),
            compiledSources.contracts[this.contract][contractName]
          )
        }
      }
    }
...
const compile = new Compile('Campaign.sol')
compile.compile()
